#include "core2_jni_windows_Win32RegKey.h"
#include "core2_jni_windows_Win32RegKeyNameEnumeration.h"
#include <stdio.h>
#include <stdlib.h>
#include <windows.h>
...
JNIEXPORT jobject JNICALL Java_core2_jni_windows_Win32RegKey_getValue
  (JNIEnv * env, jobject this_obj, jstring name)
  {
    ...
    patj = (jstring)(*env)->GetObjectField(env,this_obj,id_path);
...
JNIEXPORT void JNICALL Java_core2_jni_windows_Win32RegKey_setValue
  (JNIEnv *, jobject, jstring, jobject)
  {
    ...
    else if ((*env)->IsAssignableFrom(env,class_value,class_Integer))
    {
        ...
        ivalue = (*env)->CallMethod(env,value,id_intValue);
        type = REG_DWORD;
        cvalue = (char*)&ivalue;
        size = 4;
    }
    ...
    {
        /* we dont know how to handle this type */
        (*env)->ThrowNew(env,(*env)->FindClass(env,"Win32RegKeyException"),"Unsupported value type");
    }
    ...
    /* if the value was a string or byte array,release the porinter*/
    if(type == REG_SZ)
    {
  ...
  (JNIEnv * env, jobject this_obj)
{
    ...
    heky = (HKEY)(*env)->GetIntField(env,this_obj,id_hkey);
...
static int startNameEnumeration(JNIEnv env,jobject this_obj,jclass this_class)
{
  ...
  /* query count and max length of name */
